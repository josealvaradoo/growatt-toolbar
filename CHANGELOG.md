...
- Add a human-focused README covering setup, the local API contract, security,
  build and packaging commands, runtime behavior, architecture, and
  troubleshooting.
- Document manual refresh cache bypass behavior introduced after 0.6.0.
...
- `KeychainManager` — macOS Keychain Services wrapper.
- `AppPreferences` — `@MainActor` unified credentials façade.
...
- `Resources/Info.plist` — static bundle metadata.
- Ad-hoc codesigning and DMG packaging via `hdiutil`.
- App icon (`Growatt-G.png` → `.icns`) generated by `make icons` and embedded in the `.app` bundle so Finder shows the icon on the `.app` and `.dmg`.
